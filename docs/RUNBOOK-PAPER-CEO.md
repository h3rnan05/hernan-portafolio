...
|---|---|
| `watchlist.json` | **GHA hunter** (dueño). El cron GHA watchlist puede seguir stagedándolo como escritor secundario (rechecks cuando el VPS no corre); overlap hunter↔watchlist GHA ya estaba aceptado. **VPS no lo commitea ni lo escribe en disco** (`MOMENTUM_WATCHLIST_VPS_STATE=1`: overlay en `/var/lib/momentum/watchlist_vps_state.json`). Limitación v1: origin no ve transiciones VPS. |
