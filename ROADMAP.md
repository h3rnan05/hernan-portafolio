...
| 9 | **AI Analyst** | genera reportes institucionales: tesis, fortalezas/debilidades/riesgos/catalizadores, por qué el modelo eligió esto y rechazó lo otro, impacto en portafolio, explicación de riesgo. Todo en lenguaje humano. **Nunca decide** | 🟡 Parcial — `news_analyst/` (28 tests, con LLM pero con guardrails duros): cruza titulares reales contra `screener/shortlist_hoy.json` y explica CON LLM solo lo que matcheó ("Why Should I Care?"). `telegram_bot/report_command.py` (`/report TICKER`, 7 tests): memo por empresa con técnico real, fundamentales, consenso de analistas y % de tenencia institucional/insider (snapshots reales de yfinance, no histórico de transacciones — eso no está disponible gratis), y noticias explicadas reutilizando `news_analyst`. Falta: tesis institucional completa (10-K/earnings calls), `/options` con Greeks reales y ranking determinístico de estrategias (ver README de `telegram_bot`), `/history`/`/diff`/`/quality` (no existen). El LLM del evaluador de ideas de Telegram sigue decidiendo, lo cual no debe (ver deuda de arquitectura) |
