...
| 9 | **AI Analyst** | genera reportes institucionales: tesis, fortalezas/debilidades/riesgos/catalizadores, por qué el modelo eligió esto y rechazó lo otro, impacto en portafolio, explicación de riesgo. Todo en lenguaje humano. **Nunca decide** | 🟡 Parcial — `news_analyst/` (nuevo, 28 tests, con LLM pero con guardrails duros): cruza titulares reales contra `screener/shortlist_hoy.json` (determinístico, sin LLM) y explica CON LLM solo lo que matcheó ("Why Should I Care?"), con filtro de palabras que descarta la explicación entera si el LLM sugiere una acción. Aplicado solo a noticias vs. shortlist, no a reportes de tesis completos todavía. El resto del agente (tesis institucional completa) sigue sin existir. El LLM del evaluador de Telegram sigue decidiendo, lo cual no debe (ver deuda de arquitectura) |
